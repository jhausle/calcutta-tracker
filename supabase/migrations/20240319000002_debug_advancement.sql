...
BEGIN
  ...
  -- Log debug info
  debug_info := format('Current game: %s, Round: %s, Region: %s', 
    current_game_number::text, current_round::text, winner_region);
  ...
  -- Log calculated next game
  debug_info := format('Next game number: %s, Should be team1: %s', 
    next_game_number::text, should_be_team1::text);
  RAISE NOTICE '%', debug_info;
  ...
  FROM games g
  JOIN rounds r ON g.round_id = r.id
  -- Join with teams to get region info for first round games
  ...
  -- Log if next game was found
  IF next_game_id IS NULL THEN
    RAISE NOTICE 'No next game found';
  ELSE
    RAISE NOTICE 'Found next game: %', next_game_id;
  END IF;
