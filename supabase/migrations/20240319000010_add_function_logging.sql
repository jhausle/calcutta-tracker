-- Create a table to log function executions
CREATE TABLE IF NOT EXISTS function_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    function_name TEXT NOT NULL,
    params JSONB,
    executed_at TIMESTAMPTZ DEFAULT NOW()
);

-- Grant insert permission to anon role
GRANT INSERT ON function_logs TO anon;

-- Drop the existing function
DROP FUNCTION IF EXISTS advance_to_next_round;

-- Recreate the function with logging
CREATE OR REPLACE FUNCTION advance_to_next_round(
  game_id UUID,
  winner_team_id UUID,
  team1_score INTEGER DEFAULT NULL,
  team2_score INTEGER DEFAULT NULL
)
RETURNS jsonb AS $$
DECLARE
  debug_info jsonb;
  game_info record;
  update_result text;
BEGIN
  -- Log the function call
  INSERT INTO function_logs (function_name, params)
  VALUES (
    'advance_to_next_round',
    jsonb_build_object(
      'game_id', game_id,
      'winner_team_id', winner_team_id,
      'team1_score', team1_score,
      'team2_score', team2_score
    )
  );

  -- First, verify the parameters
  debug_info := jsonb_build_object(
    'params_received', jsonb_build_object(
      'game_id', game_id,
      'winner_team_id', winner_team_id,
      'input_team1_score', team1_score,
      'input_team2_score', team2_score
    )
  );

  -- Get current game state
  SELECT 
    g.id,
    g.team1_id,
    g.team2_id,
    g.winner_id,
    g.team1_score as current_team1_score,
    g.team2_score as current_team2_score,
    g.game_number,
    t1.college as team1_name,
    t2.college as team2_name
  INTO game_info
  FROM games g
  LEFT JOIN teams t1 ON g.team1_id = t1.id
  LEFT JOIN teams t2 ON g.team2_id = t2.id
  WHERE g.id = game_id;

  -- Add game state to debug info
  debug_info := debug_info || jsonb_build_object(
    'current_game_state', jsonb_build_object(
      'team1_id', game_info.team1_id,
      'team2_id', game_info.team2_id,
      'team1_name', game_info.team1_name,
      'team2_name', game_info.team2_name,
      'current_winner_id', game_info.winner_id,
      'current_team1_score', game_info.current_team1_score,
      'current_team2_score', game_info.current_team2_score,
      'game_number', game_info.game_number
    )
  );

  -- Verify winner is one of the teams
  debug_info := debug_info || jsonb_build_object(
    'winner_is_team1', winner_team_id = game_info.team1_id,
    'winner_is_team2', winner_team_id = game_info.team2_id,
    'winner_is_valid', (winner_team_id = game_info.team1_id OR winner_team_id = game_info.team2_id)
  );

  -- Try to update the game with error handling
  BEGIN
    UPDATE games AS g
    SET winner_id = winner_team_id,
        team1_score = COALESCE($3, g.team1_score),
        team2_score = COALESCE($4, g.team2_score)
    WHERE g.id = game_id
    AND (g.team1_id = winner_team_id OR g.team2_id = winner_team_id);
    
    GET DIAGNOSTICS update_result = ROW_COUNT;
    
    debug_info := debug_info || jsonb_build_object(
      'rows_updated', update_result
    );
  EXCEPTION WHEN OTHERS THEN
    debug_info := debug_info || jsonb_build_object(
      'update_error', SQLERRM,
      'update_error_detail', SQLSTATE
    );
  END;

  -- Check if update worked
  debug_info := debug_info || jsonb_build_object(
    'update_success', EXISTS (
      SELECT 1 FROM games 
      WHERE id = game_id AND winner_id = winner_team_id
    )
  );

  -- Get final game state
  SELECT 
    g.winner_id,
    g.team1_score as final_team1_score,
    g.team2_score as final_team2_score
  INTO game_info
  FROM games g
  WHERE g.id = game_id;

  debug_info := debug_info || jsonb_build_object(
    'final_game_state', jsonb_build_object(
      'winner_id', game_info.winner_id,
      'team1_score', game_info.final_team1_score,
      'team2_score', game_info.final_team2_score
    )
  );

  RETURN debug_info;
END;
$$ LANGUAGE plpgsql; 