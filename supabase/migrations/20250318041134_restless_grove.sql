/*
  # Update Team Earnings View with Rounded Tax Calculation

  1. Changes
    - Round tax amount up to nearest dollar using CEIL
    - Update total investment to include rounded tax
    - Maintain decimal values for purchase prices
    
  2. Details
    - Tax is 25% on amount over $250, rounded up
    - Updates net profit calculation to account for rounded tax
*/

-- Drop the existing view
DROP VIEW IF EXISTS team_earnings;

-- Recreate the view with rounded tax calculation
  ...
  SELECT
    ...
    CASE 
      WHEN total_purchase_price > 250 THEN
        CEIL((total_purchase_price - 250) * 0.25)
      ELSE 0
    END as tax_amount
  FROM owner_totals
)
